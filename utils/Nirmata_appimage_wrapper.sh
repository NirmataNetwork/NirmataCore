...
out_file_name="${out_dir}/Nirmata.desktop"
...
call_app()
{
  ...
  usr/bin/Nirmata "$@"
  if [ $? -ne 0 ]; then
    echo $'\n\n\x1b[1mIf Nirmata fails to launch, it might need to install xinerama extension for the X C Binding with this command:\n\x1b[2m   sudo apt-get install libxcb-xinerama0\n\n'
  fi
...
create_desktop_icon()
{
    ...
    rm -f "${out_dir}/Nirmata.png"
    rm -f $target_file_name
    cp -Rv "${APPDIR}/usr/share/icons/hicolor/256x256/apps/Nirmata.png" "${out_dir}/Nirmata.png"
    ...
    echo Icon=${out_dir}/Nirmata.png | tee -a $target_file_name > /dev/null
    ...
    echo "StartupWMClass=Nirmata" | tee -a $target_file_name  > /dev/null
}
