  ...
  m_offline_mode(false)
{
  ...
  m_cmd_binder.set_handler("show_staking_history", boost::bind(&simple_wallet::show_staking_history, this, _1), "show_staking_history [2] - Show staking transfers, if option provided - number of days for history to display");
  ...
  m_cmd_binder.set_handler("show_seed", boost::bind(&simple_wallet::show_seed, this, _1), "Display secret 24-26 word phrase that could be used to recover this wallet");
...
bool simple_wallet::validate_wrap_status(uint64_t amount)
{
  ...
  bool r = epee::net_utils::invoke_http_json_remote_command2("#", req, res, http_client, 10000);
  if (!r)
  {
  ...
  uint64_t nirmata_needed_for_wrap = std::stoll(res.tx_cost.nirmata_needed_for_erc20);
  if (amount <= nirmata_needed_for_wrap)
  {
    fail_msg_writer() << "Too small amount to cover ERC20 fee. ERC20 cost is: " 
      << print_money(nirmata_needed_for_wrap) << " NiR" <<
  ...
  if (amount > unwrapped_coins_left)
  {
    fail_msg_writer() << "Amount is bigger than ERC20 tokens left available: "
      << print_money(unwrapped_coins_left) << " wNiR";
    return false;
  }
  
  success_msg_writer(false) << "You'll receive estimate " << print_money(amount - nirmata_needed_for_wrap) << " wNiR (" << print_money(nirmata_needed_for_wrap)<< " NiR will be used to cover ERC20 fee)";
...
bool simple_wallet::transfer(const std::vector<std::string> &args_)
{
  ...
  for (size_t i = 0; i < local_args.size(); i += 2) 
  {
    ...
    if (is_address_like_wrapped(local_args[i]))
    {
      ...
      success_msg_writer(false) << "This transaction will create wNiR (\"Wrapped NiR\") which will be sent to the specified address on the Ethereum network.";
  ...
  try
  {
    ...
    if (!m_wallet->is_watch_only())
    {
      if(wrapped_transaction)
        success_msg_writer(true) << "Money successfully sent to wNiR custody wallet, transaction " << get_transaction_hash(tx) << ", " << get_object_blobsize(tx) << " bytes";
      else
        success_msg_writer(true) << "Money successfully sent, transaction " << get_transaction_hash(tx) << ", " << get_object_blobsize(tx) << " bytes";
    }
    else
    {
      success_msg_writer(true) << "Transaction prepared for signing and saved into \"nirmata_tx_unsigned\" file, use full wallet to sign transfer and then use \"submit_transfer\" on this wallet to broadcast the transaction to the network";
    }
  }
  catch (const tools::error::daemon_busy&)
  {
...
bool simple_wallet::sweep_below(const std::vector<std::string> &args)
{
  ...
  try
  {
    ...
    std::string filename = "nirmata_tx_unsigned";
...
#endif
{
  ...
  if (command_line::has_arg(vm, tools::wallet_rpc_server::arg_rpc_bind_port))
  {
    ...
    LOG_PRINT_L0("Starting wallet rpc server");
    wrpc.run(false, offline_mode, miner_address);
